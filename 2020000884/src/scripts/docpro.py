...
class GRU(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, id_dict_size=7, withuser=True):
        super().__init__()
...
class QTerm(nn.Module):
    def __init__(self, d_word_vec, q_dict_size, user_dim):
        super().__init__()
        self.q_dict_size = q_dict_size
        self.doc_self_attention = SelfAttention(hidden_size=d_word_vec)
        self.atten_net = AttentionNet(q_dict_size=q_dict_size, d_word_vec=d_word_vec)
        self.gru = GRU(input_size=d_word_vec, hidden_size=user_dim, withuser=False)
    ...
    def forward(
        ...
        """QDEncoder
               ...
               Args:
                  ...
                  [batch_size, max_session_num, max_session_length, d_seq_length, d_word_vec]
              Returns:
              qt_output_long (tensor): [batch_size, max_session_num-1, d_word_vec]
              qt_output_short (tensor): [batch_size, d_word_vec]
              lst_h: [batch_size, user_dim]
               """
        ...
        session_num = qt_output.shape[-2]
        qt_output_long, qt_output_short = torch.split(qt_output,[session_num-1, 1], dim = -2)
        qt_output_short =qt_output_short.squeeze(-2)
        _, lst_h = self.gru(input=qt_output_long)  # lst_h: [batch_size, user_dim]

        return lst_h, qt_output_long, qt_output_short
...
class DOCP(nn.Module):
    def __init__(self, batch_size, user_dim=40, d_word_vec=100, feature_size=110):
        ...
        self.qt_module = QTerm(d_word_vec=self.d_word_vec, q_dict_size=self.q_dict_size, user_dim=self.user_dim)
        ...
        self.gru = GRU(input_size=self.qd_dim, hidden_size=self.user_dim, id_dict_size=self.id_dict_size)
        # hidden_size: query dict size, embed_dim: in and out put dim
        self.atten_net = AttentionNet(q_dict_size=self.q_dict_size, d_word_vec=self.user_dim)
        ...
        self.proj_qwithp = nn.Sequential(nn.Linear(self.q_dict_size+1, self.q_dict_size), nn.Tanh())
    ...
    def forward(
        ...
        qt_profile_long,_, qt_profile_short = self.qt_module(q=q, docs=docs)
        # qt_profile_long(lst_h): [batch_size, user_dim] qt_profile_short: [batch_size, d_word_vec]

        qd_output = self.qd_encoder(queries=history_q, docs=docs)
        # input: [batch_size, max_session_num, max_session_length, q_dict_size+d_word_vec]
        ...
        qd_output = self.additive_atten(qd_output)
        # input: [B, qd_dim]
        qd_output = qd_output.reshape(-1, max_session_num, qd_dim)
        # input: [B, max_session_num, qd_dim]

        session_num = qd_output.shape[-2]
        # split session_num-1,1
        qd_output_long, qd_output_short = torch.split(qd_output,[session_num-1, 1], dim = -2)
        qd_output_short = qd_output_short.squeeze(-2)
        qd_profile_short = self.proj_qdprofile_short(qd_output_short)

        #  qd_profile_short: [batch_size, qd_dim]
        userid = userid.float()
        gru_output, _ = self.gru(input=qd_output_long, input_h=userid)  # gru_output: [batch_size, max_session_num, hidden_size]

        qd_profile_long = self.atten_net(input_x=gru_output, input_q=q)
        qd_profile_long = self.proj_userprofile(qd_profile_long)

        qt_profile_long = self.proj_userprofile(qt_profile_long)
        qt_profile_short = self.proj_qtprofile_short(qt_profile_short)

        user_profile = qd_profile_long + qd_profile_short + qt_profile_long + qt_profile_short

        # user_profile:[batch_size, hidden_size]
        ...
        user_mo = torch.sqrt(torch.sum(torch.square(user_profile), -1))
        d1_mo = torch.sqrt(torch.sum(torch.square(d1), -1))
        d2_mo = torch.sqrt(torch.sum(torch.square(d2), -1))

        score_1 = torch.unsqueeze((torch.sum(user_profile * d1, -1)) / (user_mo * d1_mo + 1), -1)
        features1 = self.proj_feature(features1) # [batch_size,1]
        score_1 = score_1 + features1
        # [batch_size, 1] +1?
        score_2 = torch.unsqueeze((torch.sum(user_profile * d2, -1)) / (user_mo * d2_mo + 1), -1)
        features2 = self.proj_feature(features2) # [batch_size,1]
        score_2 = score_2 + features2
